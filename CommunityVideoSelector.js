...
const CommunityVideoSelector = ({
  ...
  const handleSubmit = async () => {
    const videoId = Utils.extractYouTubeVideoId(submitUrl);
    if (!videoId) {
      Spicetify.showNotification(I18n.t("communityVideo.invalidUrl"), true);
      return;
    }
    ...
    try {
      ...
      if (!validation.valid) {
        ...
        Spicetify.showNotification(errorMsg, true);
  ...
  const handleApply = (video) => {
    ...
    Spicetify.showNotification(I18n.t("communityVideo.applied"));
  };
  ...
  const executeDelete = async () => {
    ...
    try {
      ...
      if (result) {
        Spicetify.showNotification(I18n.t("communityVideo.deleted"));
        // 목록에서 제거
        ...
        Spicetify.showNotification(I18n.t("communityVideo.deleteError"), true);
      }
    } catch (e) {
