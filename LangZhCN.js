// Simplified Chinese language file for ivLyrics
window.LANG_ZH_CN = {
  "meta": {
    "language": "简体中文",
    "code": "zh-CN",
    "author": "ivLyrics"
  },
  "tabs": {
    "general": "常规",
    "appearance": "外观",
    "behavior": "行为",
    "advanced": "高级",
    "fullscreen": "全屏",
    "debug": "调试",
    "about": "关于",
    "searchResults": "搜索结果"
  },
  "search": {
    "placeholder": "搜索设置...",
    "noResults": "未找到结果",
    "noResultsDesc": "请尝试其他搜索词",
    "resultCount": "{count}个结果",
    "clear": "清除",
    "inSection": "在{section}中"
  },
  "sections": {
    "language": "语言",
    "visualEffects": "视觉效果",
    "visualEffectsSubtitle": "自定义歌词界面的视觉元素",
    "syncMode": "同步模式",
    "syncModeSubtitle": "配置歌词同步方式",
    "typography": "字体样式",
    "typographySubtitle": "调整歌词文本的样式",
    "textShadow": "文字阴影",
    "textShadowSubtitle": "为歌词文本添加阴影效果",
    "updateCheck": "检查更新",
    "backgroundOpacity": "背景不透明度",
    "contentWidth": "内容宽度",
    "japaneseConversion": "日语转换",
    "displayContent": "显示内容",
    "autoTranslation": "自动翻译",
    "lyrics": "歌词",
    "playBar": "播放栏按钮",
    "autoScroll": "自动滚动",
    "animation": "动画",
    "karaoke": "卡拉OK",
    "syncedLyricsContent": "同步歌词内容",
    "unsyncedLyricsContent": "非同步歌词内容",
    "customPreview": "自定义预览",
    "trackSyncInfo": "曲目同步信息",
    "customDatabase": "自定义数据库",
    "debug": "调试",
    "github": "GitHub",
    "maintainers": "维护者",
    "contributors": "贡献者",
    "fadWarningTitle": "⚠️ 全屏扩展程序已激活",
    "fadWarningDesc": "在全屏扩展程序激活时不支持此功能。",
    "fadWarningTip": "请在全屏扩展程序的设置中更改对齐方式。",
    "desktopOverlay": "桌面悬浮窗",
    "desktopOverlaySubtitle": "将歌词显示为桌面悬浮窗"
  },
  "overlay": {
    "enabled": {
      "label": "启用桌面悬浮窗",
      "desc": "将歌词发送到桌面悬浮窗应用"
    },
    "port": {
      "label": "连接端口",
      "desc": "与悬浮窗应用连接的端口号 (1024-65535)"
    },
    "portSaved": "端口已保存",
    "portInvalid": "无效的端口号 (1024-65535)",
    "status": {
      "connected": "✓ 已连接",
      "disconnected": "未连接",
      "checking": "检查中..."
    },
    "openApp": "打开应用",
    "download": "下载应用",
    "downloadDesc": "如果未安装悬浮窗应用，请下载"
  },
  "settings": {
    "language": {
      "label": "语言",
      "desc": "选择扩展程序的语言"
    },
    "alignment": {
      "label": "对齐方式",
      "desc": "选择歌词文本的对齐位置",
      "options": {
        "left": "左对齐",
        "center": "居中",
        "right": "右对齐"
      }
    },
    "noise": {
      "label": "噪点覆盖",
      "desc": "在背景上添加胶片颗粒效果"
    },
    "albumBgBlur": {
      "label": "专辑背景模糊",
      "desc": "调整专辑背景的模糊强度"
    },
    "blurGradientBackground": {
      "label": "模糊渐变背景",
      "desc": "应用从专辑封面颜色提取的模糊渐变背景"
    },
    "colorful": {
      "label": "彩色背景",
      "desc": "启用基于专辑颜色的动态背景"
    },
    "gradientBackground": {
      "label": "专辑封面背景",
      "desc": "使用当前播放的专辑封面作为背景（在全屏模式下可能无法正常工作）"
    },
    "solidBackground": {
      "label": "纯色背景",
      "desc": "使用自定义纯色作为背景"
    },
    "solidBackgroundColor": {
      "label": "纯色背景颜色",
      "desc": "选择用于纯色背景的颜色"
    },
    "videoBackground": {
      "label": "视频背景",
      "desc": "使用YouTube视频作为背景（测试版）"
    },
    "videoHelper": {
      "label": "使用辅助程序",
      "desc": "在本地下载并播放YouTube视频。无需登录即可观看视频。",
      "info": "💡 YouTube视频无法加载？\n\nGoogle限制部分用户在未登录的情况下播放YouTube视频。\n使用辅助程序可以正常加载视频。",
      "download": "下载辅助程序",
      "checkConnection": "检查连接",
      "connected": "已连接到辅助程序",
      "disconnected": "无法连接到辅助程序",
      "status": {
        "connected": "已连接",
        "disconnected": "未连接",
        "checking": "检查中..."
      }
    },
    "lyricsHelper": {
      "label": "使用歌词助手",
      "desc": "发送曲目和歌词信息到助手。外部程序可通过 API 访问。",
      "sectionTitle": "助手集成",
      "sectionSubtitle": "将歌词发送到外部助手应用",
      "info": "助手应用允许外部程序显示同步歌词",
      "download": "下载助手",
      "connected": "助手已连接",
      "disconnected": "助手未连接",
      "status": {
        "checking": "检查中...",
        "connected": "已连接",
        "disconnected": "未连接"
      }
    },
    "videoBlur": {
      "label": "视频模糊",
      "desc": "调整视频背景的模糊强度 (0-40px)"
    },
    "videoCover": {
      "label": "视频填充",
      "desc": "拉伸视频以填满屏幕（可能会裁剪上下或左右）"
    },
    "backgroundBrightness": {
      "label": "背景亮度",
      "desc": "调整背景亮度级别 (0-100%)"
    },
    "solidBackgroundWarning": "使用纯色背景时，背景亮度调节不会生效。",
    "updateCheck": {
      "label": "检查更新",
      "desc": "自动检查是否有新更新",
      "info": "更新检查最多每24小时执行一次。"
    },
    "backgroundOpacity": {
      "label": "封面背景不透明度",
      "desc": "设置封面背景的不透明度"
    },
    "contentWidth": {
      "label": "内容宽度",
      "desc": "设置全屏和非全屏模式下的内容宽度"
    },
    "japaneseConversion": {
      "label": "日语转换",
      "desc": "设置日语歌词的转换格式",
      "info": "此设置仅适用于同步和非同步歌词，不适用于卡拉OK歌词。",
      "options": {
        "disabled": "禁用",
        "romaji": "罗马字",
        "furigana": "振假名",
        "hiragana": "平假名",
        "katakana": "片假名"
      }
    },
    "displayContent": {
      "label": "显示内容",
      "desc": "与歌词一起显示附加内容",
      "options": {
        "disabled": "禁用",
        "romanization": "罗马化",
        "translation": "翻译"
      }
    },
    "autoTranslation": {
      "label": "自动翻译",
      "desc": "当歌词没有翻译时自动提供翻译",
      "options": {
        "disabled": "禁用（默认）",
        "google": "谷歌翻译",
        "microsoft": "微软翻译"
      }
    },
    "lyricsMode": {
      "label": "歌词模式",
      "options": {
        "karaoke": "卡拉OK",
        "synced": "同步",
        "unsynced": "非同步",
        "genius": "Genius"
      }
    },
    "playBarButton": {
      "label": "播放栏按钮",
      "desc": "在播放栏显示歌词按钮",
      "options": {
        "disabled": "禁用",
        "normal": "普通",
        "fullscreen": "全屏"
      }
    },
    "autoScroll": {
      "label": "自动滚动",
      "desc": "启用自动滚动。手动滚动后5秒恢复自动滚动。"
    },
    "animation": {
      "label": "动画",
      "desc": "显示卡拉OK高亮行以外的动画"
    },
    "karaoke": {
      "label": "卡拉OK前奏倒计时",
      "desc": "以数字显示前奏倒计时。禁用时显示为点。"
    },
    "blurInactive": {
      "label": "模糊非活动行",
      "desc": "在同步歌词中模糊非活动行",
      "info": "启用此选项可能会导致性能问题。"
    },
    "fade": {
      "label": "模糊和渐变",
      "desc": "在歌词顶部和底部显示模糊和渐变",
      "info": "启用此选项可能会导致性能问题。"
    },
    "syncedAsFallback": {
      "label": "同步歌词备用",
      "desc": "当卡拉OK歌词不可用时回退到同步歌词"
    },
    "unsyncedAsFallback": {
      "label": "非同步歌词备用",
      "desc": "当同步歌词不可用时回退到非同步歌词"
    },
    "preferUnsynced": {
      "label": "优先非同步歌词",
      "desc": "当同步和非同步歌词都可用时优先使用非同步歌词。",
      "info": "* 某些非同步歌词可能包含同步歌词中没有的内容。"
    },
    "customPreview": {
      "label": "自定义预览",
      "desc": "自定义歌词屏幕预览图像。图像以300x150px显示。",
      "placeholder": "输入图片URL"
    },
    "trackSyncInfo": {
      "label": "曲目同步信息",
      "desc": "管理与曲目关联的同步信息"
    },
    "customDatabase": {
      "label": "自定义数据库",
      "desc": "设置自定义数据库URL",
      "placeholder": "输入自定义数据库URL"
    },
    "debugVerbose": {
      "label": "调试详细信息",
      "desc": "向控制台输出更详细的信息"
    },
    "cache": {
      "deleteAll": "删除所有缓存的歌词",
      "noCache": "没有缓存的歌词"
    },
    "colors": {
      "customColor": "自定义",
      "showMore": "更多 ▼",
      "showLess": "收起 ▲",
      "black": "黑色",
      "charcoal": "炭灰色",
      "darkSlate": "深石板色",
      "gray": "灰色",
      "darkNavy": "深海军蓝",
      "navy": "海军蓝",
      "royalBlue": "皇家蓝",
      "sky": "天蓝色",
      "indigo": "靛蓝色",
      "purple": "紫色",
      "fuchsia": "紫红色",
      "pink": "粉红色",
      "wine": "酒红色",
      "red": "红色",
      "orange": "橙色",
      "amber": "琥珀色",
      "gold": "金色",
      "lime": "青柠色",
      "green": "绿色",
      "emerald": "翡翠绿",
      "teal": "青色",
      "cyan": "青色",
      "brown": "棕色",
      "chocolate": "巧克力色"
    },
    "solidBackgroundInUse": "ℹ️ 正在使用纯色背景",
    "fontPlaceholder": "输入字体名称（如：Arial, 微软雅黑）"
  },
  "buttons": {
    "save": "保存",
    "cancel": "取消",
    "reset": "重置",
    "import": "导入",
    "export": "导出",
    "open": "打开",
    "close": "关闭",
    "apply": "应用",
    "confirm": "确认",
    "delete": "删除",
    "edit": "编辑",
    "add": "添加",
    "remove": "移除",
    "clear": "清除",
    "refresh": "刷新",
    "settings": "设置",
    "fullscreen": "全屏",
    "exitFullscreen": "退出全屏"
  },
  "menu": {
    "translation": "转换",
    "settings": "设置",
    "syncAdjust": "同步调整",
    "syncAdjustTitle": "歌词同步调整",
    "copyLyrics": "复制歌词",
    "searchLyrics": "搜索歌词",
    "editLyrics": "编辑歌词",
    "shareImage": "分享歌词图片",
    "translationOptions": "转换选项",
    "translationOptionsSubtitle": "配置歌词的发音和翻译显示",
    "detectedLanguage": "检测到的语言",
    "detectedLanguageInfo": "当前播放歌曲的语言。转换选项会根据语言不同而变化。",
    "unknownLanguage": "未知",
    "autoDetect": "自动检测",
    "overrideLanguage": "手动设置语言",
    "overrideLanguageInfo": "手动设置此曲目的语言。将使用选择的语言代替自动检测。",
    "pronunciation": "发音",
    "pronunciationInfo": "显示原文歌词的罗马化发音",
    "translationLabel": "翻译",
    "translationInfo": "显示翻译后的歌词",
    "apiSettings": "API设置",
    "apiSettingsSubtitle": "配置Gemini API",
    "apiKeySettings": "API密钥设置",
    "apiKeySettingsInfo": "点击此处配置Gemini API密钥",
    "openSettings": "打开设置",
    "translationSettings": "转换设置",
    "regenerateTranslation": "重新生成翻译",
    "fullscreen": "全屏"
  },
  "messages": {
    "noLyrics": "没有可用的歌词",
    "loading": "加载中...",
    "error": "发生错误",
    "saved": "已保存",
    "copied": "已复制",
    "deleted": "已删除",
    "updated": "已更新",
    "applied": "已应用",
    "noTrackInfo": "没有曲目信息",
    "syncedLyricsCopied": "同步歌词已复制到剪贴板。",
    "unsyncedLyricsCopied": "非同步歌词已复制到剪贴板。"
  },
  "update": {
    "newVersion": "新版本",
    "available": "可用！",
    "notes": "更新说明",
    "update": "更新",
    "dismiss": "稍后",
    "alreadyLatest": "已是最新版本",
    "checkFailed": "检查更新失败",
    "versionChange": "版本",
    "copyCommand": "复制命令",
    "releaseNotes": "发行说明",
    "expand": "详情",
    "collapse": "收起",
    "copied": "已复制"
  },
  "fullscreen": {
    "title": "全屏",
    "background": "背景",
    "backgroundOptions": {
      "albumArt": "专辑封面",
      "animatedCanvas": "动画画布",
      "artistArt": "艺术家图片",
      "staticColor": "纯色"
    },
    "showExtraControls": {
      "label": "显示额外控件",
      "desc": "在全屏模式下显示额外控件"
    },
    "contextDisplay": {
      "label": "上下文显示",
      "desc": "在全屏模式下显示当前播放的上下文"
    },
    "upcomingLyrics": {
      "label": "显示即将播放的歌词",
      "desc": "在全屏模式下预览即将播放的歌词"
    },
    "volumeDisplay": {
      "label": "音量显示",
      "desc": "在全屏模式下显示音量控制"
    },
    "controls": {
      "like": "喜欢",
      "unlike": "取消喜欢",
      "shuffle": "随机播放",
      "previous": "上一曲",
      "next": "下一曲",
      "nextTrackLabel": "下一曲",
      "play": "播放",
      "pause": "暂停",
      "repeatOff": "关闭重复",
      "repeatAll": "重复全部",
      "repeatOne": "单曲循环",
      "mute": "静音",
      "unmute": "取消静音",
      "share": "复制分享链接",
      "shareCopied": "🔗 分享链接已复制"
    },
    "queue": {
      "title": "播放队列",
      "recentlyPlayed": "最近播放",
      "nowPlaying": "正在播放",
      "upNext": "即将播放",
      "empty": "队列为空",
      "noRecent": "没有最近播放记录"
    },
    "contextType": {
      "playlist": "播放列表",
      "album": "专辑",
      "artist": "艺术家",
      "collection": "喜欢的歌曲",
      "station": "电台"
    }
  },
  "about": {
    "version": "版本",
    "sourceCode": "源代码",
    "reportIssue": "报告问题",
    "changelog": "更新日志",
    "license": "许可证",
    "credits": "致谢"
  },
  "translationMenu": {
    "none": "无",
    "disabled": "禁用",
    "romaji": "罗马字",
    "hiragana": "平假名",
    "katakana": "片假名",
    "furigana": "振假名",
    "romanization": "罗马化",
    "translation": "翻译",
    "google": "谷歌翻译",
    "microsoft": "微软翻译",
    "geminiRomaji": "发音",
    "geminiKo": "翻译",
    "romajiGemini": "罗马字 (Gemini)",
    "koGemini": "韩语 (Gemini)",
    "simplifiedChinese": "简体中文",
    "traditionalChineseHK": "繁体中文（香港）",
    "traditionalChineseTW": "繁体中文（台湾）",
    "pinyin": "拼音"
  },
  "notifications": {
    "languageChanged": "语言已更改。刷新后生效。",
    "settingsSaved": "设置已保存",
    "settingsReset": "设置已重置",
    "exportSuccess": "导出成功",
    "importSuccess": "导入成功",
    "importFailed": "导入设置失败",
    "syncDataCleared": "同步数据已清除",
    "syncDataClearFailed": "清除同步数据失败",
    "lyricsCopied": "✓ 歌词已复制到剪贴板",
    "lyricsCopyFailed": "复制歌词到剪贴板失败",
    "translationCopied": "翻译已复制到剪贴板",
    "translationCopyFailed": "复制翻译到剪贴板失败",
    "secondTranslationCopied": "第二翻译已复制到剪贴板",
    "secondTranslationCopyFailed": "复制第二翻译到剪贴板失败",
    "installCommandCopied": "安装命令已复制",
    "copyFailed": "复制失败",
    "memoryCacheCleared": "✓ 内存缓存已清除",
    "localCacheCleared": "✓ 所有本地缓存已清除",
    "localCacheTrackCleared": "✓ 当前曲目缓存已清除",
    "exportSuccessDesc": "设置文件已保存到下载文件夹。",
    "exportFailed": "导出设置失败",
    "importSuccessDesc": "页面即将刷新...",
    "resetSuccess": "重置完成",
    "resetSuccessDesc": "页面即将刷新...",
    "resetFailed": "重置失败",
    "updateCheckFailed": "检查更新失败",
    "checkNetworkConnection": "请检查网络连接",
    "updateAvailable": "有可用更新",
    "latestVersion": "已是最新版本",
    "noLyricsLoaded": "没有加载歌词。",
    "translationRegenerateGeminiOnly": "重新生成翻译仅适用于Gemini翻译。",
    "regeneratingTranslation": "正在重新生成翻译...",
    "translationRegenerated": "✓ 翻译已重新生成",
    "translationRegenerateFailed": "重新生成翻译失败",
    "translationCacheRemoved": "✓ 翻译缓存已清除并重新加载！",
    "translationCacheReset": "翻译缓存 {count} 项已重置",
    "tooManyTranslationRequests": "翻译请求过多。请1分钟后重试。",
    "requestingPronunciation": "正在请求发音。大约需要30秒",
    "requestingTranslation": "正在请求翻译。大约需要30秒",
    "shareImageCopied": "图片已复制到剪贴板",
    "shareImageDownloaded": "图片已下载",
    "shareImageShared": "分享成功",
    "shareImageFailed": "图片生成失败",
    "shareImageNoLyrics": "请选择要分享的歌词",
    "noTrackPlaying": "没有正在播放的曲目",
    "overlayConnected": "悬浮窗已连接",
    "romajiTranslationFailed": "罗马字/发音转换失败",
    "koreanTranslationFailed": "韩语翻译失败",
    "pinyinLibraryUnavailable": "拼音库不可用。显示原文。请允许jsDelivr或unpkg。",
    "conversionSkippedSimplified": "跳过转换：已是简体字",
    "conversionCompleted": "转换完成",
    "conversionFailed": "转换失败",
    "fileTooLarge": "文件太大：最大大小为1MB",
    "noValidLyricsInFile": "在文件中找不到有效的歌词",
    "lyricsLoadedFromFile": "从文件加载了 {types} 歌词",
    "lyricsLoadFailed": "歌词加载失败：无效的文件格式",
    "fileReadFailed": "文件读取失败：文件可能已损坏"
  },
  "misc": {
    "and": "和",
    "or": "或",
    "enabled": "已启用",
    "disabled": "已禁用",
    "default": "默认",
    "custom": "自定义",
    "none": "无",
    "auto": "自动",
    "manual": "手动",
    "on": "开",
    "off": "关",
    "yes": "是",
    "no": "否",
    "items": "项",
    "seconds": "秒",
    "minutes": "分钟"
  },
  "settingsAdvanced": {
    "patchNotes": {
      "empty": "没有更新说明。"
    },
    "donate": {
      "title": "请开发者喝杯咖啡",
      "button": "捐赠"
    },
    "syncMode": {
      "title": "同步模式",
      "subtitle": "紧凑同步模式的显示选项",
      "linesBefore": {
        "label": "之前的行数",
        "desc": "当前歌词之前显示的行数"
      },
      "linesAfter": {
        "label": "之后的行数",
        "desc": "当前歌词之后显示的行数"
      },
      "fadeoutBlur": {
        "label": "淡出模糊效果",
        "desc": "对非活动歌词应用模糊效果"
      },
      "highlightMode": {
        "label": "高亮模式",
        "desc": "只高亮显示当前播放的歌词，其他歌词变暗"
      },
      "highlightIntensity": {
        "label": "高亮强度",
        "desc": "调整非活动歌词的透明度（值越低越暗）"
      }
    },
    "livePreview": {
      "title": "实时预览",
      "subtitle": "即时查看您的样式设置",
      "sampleText": "这里有歌词",
      "sampleTextMixed": "여기 Lyric あります",
      "sampleTextPhonetic": "yeogi lyric arimasu"
    },
    "originalStyle": {
      "title": "原文样式",
      "subtitle": "原文歌词的字体设置",
      "fontFamily": "字体",
      "fontFamilyDesc": "选择应用于原文歌词的字体。用逗号（,）分隔多个字体按顺序应用。",
      "fontSize": {
        "label": "字体大小",
        "desc": "原文歌词的字体大小（像素）"
      },
      "fontWeight": {
        "label": "字体粗细",
        "desc": "原文歌词的字体粗细"
      },
      "opacity": {
        "label": "不透明度",
        "desc": "原文歌词的不透明度 (0-100%)"
      },
      "letterSpacing": {
        "label": "字符间距",
        "desc": "原文歌词的字符间距（像素）"
      }
    },
    "pronunciationStyle": {
      "title": "发音样式",
      "subtitle": "罗马化发音（罗马字、拼音）的字体设置",
      "fontFamily": "字体",
      "fontFamilyDesc": "选择应用于罗马化的字体。用逗号（,）分隔多个字体按顺序应用。",
      "fontSize": {
        "label": "字体大小",
        "desc": "罗马化的字体大小（像素）"
      },
      "fontWeight": {
        "label": "字体粗细",
        "desc": "罗马化的字体粗细"
      },
      "opacity": {
        "label": "不透明度",
        "desc": "罗马化的不透明度 (0-100%)"
      },
      "gap": {
        "label": "与原文的间距",
        "desc": "原文歌词与罗马化的间距"
      },
      "letterSpacing": {
        "label": "字符间距",
        "desc": "罗马化的字符间距（像素）"
      },
      "hyphenReplace": {
        "label": "连字符处理",
        "desc": "将发音中的连字符(-)替换为其他字符",
        "options": {
          "keep": "保持",
          "space": "替换为空格",
          "remove": "删除"
        }
      }
    },
    "translationStyle": {
      "title": "翻译样式",
      "subtitle": "翻译歌词的字体设置",
      "fontFamily": "字体",
      "fontFamilyDesc": "选择应用于翻译歌词的字体。用逗号（,）分隔多个字体按顺序应用。",
      "fontSize": {
        "label": "字体大小",
        "desc": "翻译歌词的字体大小（像素）"
      },
      "fontWeight": {
        "label": "字体粗细",
        "desc": "翻译歌词的字体粗细"
      },
      "opacity": {
        "label": "不透明度",
        "desc": "翻译歌词的不透明度 (0-100%)"
      },
      "gap": {
        "label": "与发音的间距",
        "desc": "罗马化与翻译的间距（像素）"
      },
      "letterSpacing": {
        "label": "字符间距",
        "desc": "翻译歌词的字符间距（像素）"
      }
    },
    "furiganaStyle": {
      "title": "注音样式",
      "subtitle": "显示在日文汉字上方的假名注音设置",
      "fontSize": {
        "label": "字体大小",
        "desc": "注音的字体大小（像素）"
      },
      "fontWeight": {
        "label": "字体粗细",
        "desc": "注音的字体粗细"
      },
      "opacity": {
        "label": "透明度",
        "desc": "注音的不透明度 (0-100%)"
      },
      "spacing": {
        "label": "间距",
        "desc": "注音与汉字之间的间距（像素）"
      }
    },
    "textShadow": {
      "title": "文字阴影",
      "subtitle": "提高可读性的阴影效果",
      "enabled": {
        "label": "阴影效果",
        "desc": "对歌词文本应用阴影效果"
      },
      "color": {
        "label": "阴影颜色",
        "desc": "阴影的颜色（HEX代码）"
      },
      "opacity": {
        "label": "阴影不透明度",
        "desc": "阴影的不透明度 (0-100%)"
      },
      "blur": {
        "label": "模糊强度",
        "desc": "阴影的模糊程度"
      }
    },
    "playback": {
      "title": "播放行为",
      "subtitle": "与播放相关的功能设置",
      "replaceButton": {
        "label": "替换默认歌词按钮",
        "info": "用ivLyrics替换Spotify的默认歌词按钮"
      },
      "replaceFullscreenButton": {
        "label": "替换全屏按钮",
        "info": "用ivLyrics全屏替换Spotify的默认全屏按钮"
      },
      "fullscreenShortcut": {
        "label": "全屏快捷键",
        "desc": "歌词全屏模式的键盘快捷键"
      },
    },
    "nowPlayingPanel": {
      "title": "正在播放面板歌词",
      "subtitle": "右侧「正在播放」面板的歌词显示设置",
      "enabled": {
        "label": "启用面板歌词",
        "desc": "在正在播放面板中显示当前歌词"
      },
      "lines": {
        "label": "歌词行数",
        "desc": "面板中显示的歌词行数（默认: 5）"
      },
      "fontScale": {
        "label": "整体字体大小比例",
        "desc": "面板歌词的整体大小比例（50%-200%）"
      },
      "fontFamily": {
        "label": "默认字体",
        "desc": "面板歌词使用的默认字体（未设置单独字体时适用）"
      },
      "originalFont": {
        "label": "原文字体",
        "desc": "原文歌词使用的字体（为空则使用默认字体，可用逗号分隔多个字体）"
      },
      "phoneticFont": {
        "label": "发音字体",
        "desc": "发音（罗马字等）使用的字体（为空则使用默认字体，可用逗号分隔多个字体）"
      },
      "translationFont": {
        "label": "翻译字体",
        "desc": "翻译歌词使用的字体（为空则使用默认字体，可用逗号分隔多个字体）"
      },
      "originalSize": {
        "label": "原文字体大小",
        "desc": "原文歌词的字体大小（px）"
      },
      "phoneticSize": {
        "label": "发音字体大小",
        "desc": "发音（罗马字等）的字体大小（px）"
      },
      "translationSize": {
        "label": "翻译字体大小",
        "desc": "翻译歌词的字体大小（px）"
      },
      "preview": "预览",
      "background": {
        "title": "背景",
        "subtitle": "面板背景样式设置",
        "type": {
          "label": "背景类型",
          "desc": "选择背景样式",
          "album": "专辑颜色",
          "custom": "自定义颜色",
          "gradient": "专辑渐变"
        },
        "color": {
          "label": "背景颜色",
          "desc": "自定义背景颜色"
        },
        "gradient1": {
          "label": "渐变色1",
          "desc": "第一个渐变颜色"
        },
        "gradient2": {
          "label": "渐变色2",
          "desc": "第二个渐变颜色"
        },
        "opacity": {
          "label": "背景不透明度",
          "desc": "背景不透明度（0%=透明，100%=不透明）"
        }
      },
      "border": {
        "title": "边框",
        "subtitle": "面板边框样式设置",
        "enabled": {
          "label": "启用边框",
          "desc": "在面板周围显示边框"
        },
        "color": {
          "label": "边框颜色",
          "desc": "边框的颜色"
        },
        "opacity": {
          "label": "边框不透明度",
          "desc": "边框不透明度（0%=透明，100%=不透明）"
        }
      }
    },
    "karaokeMode": {
      "title": "卡拉OK模式",
      "subtitle": "卡拉OK风格歌词显示",
      "enabled": {
        "label": "启用卡拉OK模式",
        "desc": "在支持卡拉OK歌词的歌曲中使用卡拉OK标签。禁用时使用同步标签"
      },
      "bounce": {
        "label": "字符弹跳效果",
        "desc": "在卡拉OK模式下对当前字符应用弹跳动画"
      }
    },
    "prefetch": {
      "title": "预加载下一首",
      "subtitle": "预加载下一首歌曲的元素以减少切换延迟",
      "enabled": {
        "label": "预加载翻译/发音",
        "desc": "预先请求下一首歌曲的翻译和发音转换"
      },
      "videoEnabled": {
        "label": "预加载视频背景",
        "desc": "预先获取下一首歌曲的视频背景信息"
      }
    },
    "communitySync": {
      "title": "社区同步",
      "subtitle": "与其他用户共享歌词同步偏移",
      "enabled": {
        "label": "使用社区同步",
        "desc": "使用社区共享的同步偏移"
      },
      "autoApply": {
        "label": "自动应用",
        "desc": "当置信度足够时自动应用社区偏移"
      },
      "autoSubmit": {
        "label": "自动提交",
        "desc": "更改同步偏移时自动分享给社区"
      },
      "minConfidence": {
        "label": "最低置信度",
        "desc": "自动应用所需的最低置信度 (0.0 ~ 1.0)。置信度根据提交数量和偏移值的一致性计算。较高的值表示更多用户提交了相似的偏移值"
      }
    },
    "cacheManagement": {
      "title": "缓存管理",
      "subtitle": "管理存储的数据",
      "memoryCache": {
        "label": "清除内存缓存",
        "desc": "加载的歌词临时存储在内存中以便快速重新加载。无需重启Spotify即可清除内存缓存",
        "button": "清除缓存"
      },
      "localCache": {
        "label": "本地缓存管理 (IndexedDB)",
        "desc": "歌词、翻译、发音等存储在本地，减少重复播放时的API调用",
        "clearAll": "清除全部",
        "clearCurrent": "清除当前曲目",
        "stats": "歌词: {lyrics}个，翻译: {translations}个，元数据: {metadata}个"
      }
    },
    "languageDetection": {
      "title": "语言检测",
      "subtitle": "文本转换的语言检测设置",
      "furigana": {
        "label": "为日语汉字显示振假名",
        "desc": "在日语歌词的汉字上方显示平假名读音"
      },
      "japaneseThreshold": {
        "label": "日语检测阈值",
        "desc": "通过歌词中假名字符的比例检测日语。值越高检测越严格（百分比）"
      },
      "chineseThreshold": {
        "label": "中文检测阈值",
        "desc": "通过繁简字符的比例检测中文类型。值越高检测越严格（百分比）"
      }
    },
    "apiKeys": {
      "subtitle": "用于外部服务集成的API密钥"
    },
    "exportImport": {
      "title": "导出/导入设置",
      "subtitle": "将设置转移到另一台设备",
      "export": {
        "label": "导出设置",
        "button": "导出",
        "processing": "正在导出..."
      },
      "import": {
        "label": "导入设置",
        "button": "导入",
        "processing": "正在导入..."
      }
    },
    "resetSettings": {
      "title": "重置设置",
      "subtitle": "将所有设置恢复为默认值",
      "reset": {
        "label": "重置所有设置",
        "desc": "将所有设置恢复为默认值。此操作无法撤销",
        "button": "重置",
        "confirm": "确定要重置所有设置吗？\n\n此操作无法撤销，所有设置将恢复为默认值。\n\n点击'确定'继续。",
        "processing": "正在重置..."
      }
    },
    "fullscreenMode": {
      "title": "全屏模式",
      "subtitle": "全屏模式的布局和显示设置",
      "shortcut": {
        "desc": "全屏快捷键",
        "info": "设置切换全屏模式的键盘快捷键"
      },
      "toggleTvModeKey": {
        "desc": "切换TV模式快捷键",
        "info": "仅在全屏模式下有效。在TV模式和普通模式之间切换"
      },
      "tvMode": {
        "desc": "TV模式",
        "info": "在左下角显示专辑封面和曲目信息，歌词使用全屏"
      },
      "tvModeAlbumSize": {
        "desc": "TV模式专辑大小",
        "info": "设置TV模式下专辑封面的大小（像素）"
      },
      "splitView": {
        "desc": "分割布局",
        "info": "在全屏中左侧显示专辑封面，右侧显示歌词"
      },
      "invertPosition": {
        "desc": "反转专辑/歌词位置",
        "info": "交换专辑和歌词的位置（左↔右，或纵向模式中的上↔下）"
      },
      "showAlbumArt": {
        "desc": "显示专辑封面",
        "info": "在全屏左侧面板显示专辑封面"
      },
      "showTrackInfo": {
        "desc": "显示曲目信息",
        "info": "在全屏中显示歌曲标题和艺术家"
      },
      "trimTitle": {
        "desc": "精简标题",
        "info": "从歌曲标题中移除(Remaster)、[feat. xxx]等附加信息，使显示更简洁"
      },
      "translateMetadata": {
        "desc": "翻译标题/艺术家",
        "info": "翻译外语歌曲标题和艺术家名称（需要 Gemini API 密钥）"
      },
      "translateMetadataMode": {
        "desc": "翻译显示方式",
        "info": "选择如何显示标题和艺术家",
        "options": {
          "translated": "仅翻译",
          "romanized": "仅发音",
          "originalTranslated": "原文 + 翻译",
          "originalRomanized": "原文 + 发音",
          "all": "全部显示"
        }
      },
      "centerWhenNoLyrics": {
        "desc": "无歌词时居中显示专辑",
        "info": "当歌词不可用或正在加载时将专辑封面居中"
      },
      "infoGap": {
        "desc": "专辑封面间距",
        "info": "调整专辑封面与控件之间的间距（像素）"
      },
      "twoColumnLayout": {
        "desc": "使用双列布局"
      },
      "browserFullscreen": {
        "desc": "使用 Spotify 全屏",
        "info": "进入全屏模式时将 Spotify 切换为全屏以填满整个显示器"
      }
    },
    "normalMode": {
      "title": "普通模式布局",
      "subtitle": "配置普通全屏模式的布局（TV模式关闭时）",
      "showAlbumName": {
        "desc": "显示专辑名",
        "info": "在歌曲标题和艺术家下方显示专辑名称",
      },
    },
    "tvMode": {
      "title": "TV模式设置",
      "subtitle": "配置TV模式的显示选项",
      "showAlbumName": {
        "desc": "显示专辑名",
        "info": "在TV模式下在艺术家下方显示专辑名称",
      },
      "showControls": {
        "desc": "显示控制按钮",
        "info": "在TV模式下显示播放/暂停、上一曲/下一曲按钮",
      },
      "showProgress": {
        "desc": "显示进度条",
        "info": "在TV模式下显示播放进度和时间",
      },
    },
    "metadataDisplay": {
      "title": "标题/艺术家显示",
      "subtitle": "配置歌曲标题和艺术家的显示方式",
    },
    "fullscreenStyle": {
      "title": "全屏样式",
      "subtitle": "全屏模式的视觉元素设置",
      "albumSize": {
        "desc": "专辑封面大小",
        "info": "设置全屏中专辑封面的最大尺寸 (100-500px)"
      },
      "albumRadius": {
        "desc": "专辑封面圆角",
        "info": "设置专辑封面的圆角半径 (0-50px)"
      },
      "titleFontSize": {
        "desc": "标题字体大小",
        "info": "设置全屏中歌曲标题的字体大小 (24-72px)"
      },
      "artistFontSize": {
        "desc": "艺术家字体大小",
        "info": "设置全屏中艺术家名称的字体大小 (14-36px)"
      },
      "lyricsRightMargin": {
        "desc": "歌词右边距",
        "info": "设置全屏中歌词区域的右边距。防止居中对齐时歌词偏右 (0-300px)"
      }
    },
    "fullscreenUI": {
      "title": "全屏UI元素",
      "subtitle": "配置全屏中显示的其他UI元素",
      "showClock": {
        "desc": "显示时钟",
        "info": "在右上角显示当前时间"
      },
      "clockSize": {
        "desc": "时钟大小",
        "info": "设置时钟文本的大小 (24-72px)"
      },
      "showContext": {
        "desc": "显示播放上下文",
        "info": "在左下角显示当前播放的播放列表/专辑信息"
      },
      "showContextImage": {
        "desc": "显示上下文图片",
        "info": "显示播放列表/专辑缩略图"
      },
      "showNextTrack": {
        "desc": "下一曲预览",
        "info": "在歌曲结束前显示下一曲信息（广播风格）"
      },
      "nextTrackTime": {
        "desc": "下一曲显示时间",
        "info": "设置歌曲结束前多少秒显示下一曲 (5-30秒)",
        "unit": "秒"
      },
      "showControls": {
        "desc": "显示播放器控件",
        "info": "显示播放/暂停、上一曲/下一曲、随机播放、重复、喜欢按钮"
      },
      "showVolume": {
        "desc": "显示音量控制",
        "info": "在播放器控件中显示音量滑块"
      },
      "showProgressBar": {
        "desc": "显示进度条",
        "info": "显示当前播放位置和总时长的进度条"
      },
      "showLyricsProgress": {
        "desc": "显示歌词进度",
        "info": "显示当前歌词行号和总行数"
      },
      "showQueue": {
        "desc": "显示队列面板",
        "info": "当鼠标悬停在屏幕右侧时显示播放队列"
      }
    },
    "controllerStyle": {
      "title": "控制器样式",
      "subtitle": "配置播放器控制器外观",
      "buttonSize": {
        "desc": "控制按钮大小",
        "info": "设置播放、上一曲/下一曲按钮的大小 (28-48px)"
      },
      "background": {
        "desc": "控制器背景",
        "info": "为控制器添加半透明背景"
      }
    },
    "autoHide": {
      "title": "自动隐藏",
      "subtitle": "鼠标不活动时自动隐藏UI",
      "enabled": {
        "info": "鼠标不移动时控制器和信息将自动隐藏",
        "desc": "自动隐藏UI"
      },
      "delay": {
        "desc": "自动隐藏延迟",
        "info": "鼠标不活动后UI隐藏的时间 (1-10秒)"
      }
    },
    "tmiStyle": {
      "title": "TMI样式",
      "subtitle": "点击专辑封面时显示的TMI弹窗样式设置",
      "fontSize": {
        "desc": "TMI字体大小",
        "info": "调整TMI弹窗的整体字体大小 (80-150%)"
      }
    },
    "aboutTab": {
      "appInfo": {
        "title": "应用信息"
      },
      "developer": "开发者：",
      "originalProject": "原始项目：",
      "thanks": "感谢所有对开源项目的贡献者。",
      "clientInfo": {
        "title": "客户端信息",
        "subtitle": "此客户端的唯一标识符",
        "description": "这是为账户关联自动生成的唯一标识符。此值无法修改，每个客户端唯一分配。请注意不要泄露此值。",
        "copied": "客户端ID已复制",
        "copyFailed": "复制失败",
        "copy": "复制"
      },
      "update": {
        "title": "更新",
        "subtitle": "检查最新版本",
        "checkUpdate": {
          "desc": "检查最新版本",
          "button": "检查更新",
          "checking": "检查中..."
        },
        "copied": "已复制",
        "installCopied": "安装命令已复制",
        "copyFailed": "复制失败"
      },
      "patchNotes": {
        "title": "更新说明",
        "subtitle": "最新更新历史",
        "loading": "正在加载更新说明..."
      },
      "subtitle": "关于ivLyrics",
      "appDescription": "包含各种功能的歌词扩展",
      "versionPrefix": "版本",
      "viewOnGithub": "在GitHub上查看",
      "patchNotesLoadFailed": "无法加载更新说明",
      "checkGithubReleases": "请查看GitHub发布页面",
      "visitGithub": "访问GitHub仓库",
      "joinDiscord": "加入Discord服务器"
    },
    "debugTab": {
      "title": "调试信息",
      "subtitle": "报告错误时与开发者共享的信息",
      "currentTrack": "当前曲目信息",
      "trackInfo": "曲目信息",
      "lyricsInfo": "歌词信息",
      "settingsInfo": "设置信息",
      "noLyrics": "没有歌词信息",
      "loading": "正在加载信息...",
      "refresh": "刷新",
      "copyToClipboard": "复制到剪贴板",
      "copied": "已复制！",
      "copyFailed": "复制失败",
      "sendToDiscord": "报告到Discord",
      "rawJson": "JSON数据"
    },
    "api": {
      "title": "API设置",
      "getApiKey": {
        "desc": "获取API密钥",
        "info": "您可以在Google AI Studio免费获取Gemini API密钥",
        "button": "打开API密钥页面"
      },
      "geminiKey": {
        "desc": "Gemini API密钥",
        "info": "需要API密钥才能使用由Google Gemini AI驱动的歌词翻译功能"
      }
    },
    "update": {
      "title": "更新",
      "subtitle": "检查最新版本",
      "checkUpdate": {
        "desc": "检查最新版本",
        "button": "检查更新",
        "checking": "检查中..."
      },
      "copied": "已复制",
      "installCopied": "安装命令已复制",
      "copyFailed": "复制失败",
      "currentVersionInfo": "当前版本：v{version}。在GitHub上检查新更新",
      "checkFailedWithError": "检查更新失败：{error}"
    }
  },
  "syncAdjust": {
    "info": "将滑块向右移动会使歌词更快。",
    "reset": "重置",
    "communityTitle": "社区同步",
    "communityOffset": "偏移",
    "submissions": "提交数",
    "confidenceHigh": "高置信度",
    "confidenceMedium": "中置信度",
    "confidenceLow": "低置信度",
    "applyCommunity": "应用",
    "submitMine": "提交我的偏移",
    "submitting": "提交中...",
    "submitSuccess": "已提交到社区",
    "submitFailed": "提交失败",
    "noData": "没有社区数据",
    "loading": "加载中...",
    "feedbackGood": "偏移量准确",
    "feedbackBad": "偏移量不准确",
    "cannotFeedbackOwnSubmission": "无法评价自己提交的偏移量",
    "feedbackPositiveSuccess": "感谢您的肯定反馈！",
    "feedbackNegativeSuccess": "感谢您的反馈。我们会改进！",
    "feedbackFailed": "反馈提交失败",
    "autoSubmitEnabled": "⚡ 自动提交已启用"
  },
  "playbarButton": {
    "label": "ivLyrics"
  },
  "videoBackground": {
    "loading": "正在加载视频信息...",
    "notFound": "找不到视频。",
    "error": "发生错误。",
    "loadingMessage": "正在加载视频背景... 大约需要30秒。",
    "downloadingVideo": "正在下载视频...",
    "downloading": "下载中: {percent}%",
    "processing": "正在处理视频...",
    "checking": "正在检查视频...",
    "preparing": "正在准备视频下载...",
    "downloadComplete": "视频下载完成！",
    "helperNotConnected": "无法连接到助手程序。请确保程序正在运行。",
    "helperError": "助手程序发生错误。",
    "tryingCookiesFile": "正在尝试使用cookies.txt文件...",
    "tryingBrowserCookies": "正在尝试使用{browser}浏览器Cookie...",
    "checkingWithCookiesFile": "正在使用cookies.txt检查视频...",
    "checkingWithBrowserCookies": "正在使用{browser}Cookie检查视频...",
    "checkingAvailability": "正在检查视频可用性...",
    "ageRestrictedNoCookies": "年龄限制视频。未找到cookies.txt文件或支持的浏览器。请在设置中配置cookies.txt文件。",
    "ageRestrictedFailed": "年龄限制视频。请在设置中配置有效的cookies.txt文件。",
    "videoAlreadyDownloaded": "视频已下载",
    "videoAvailable": "视频可用",
    "videoNotDownloaded": "视频未下载"
  },
  "translator": {
    "missingApiKey": "未设置Gemini API密钥。请在设置中输入API密钥。",
    "invalidApiKeyFormat": "API密钥格式无效。Gemini API密钥必须以'AIza'开头。",
    "invalidRequestFormat": "请求格式无效。请检查您的API密钥。",
    "invalidApiKey": "API密钥无效。请在设置中检查您的Gemini API密钥。",
    "accessForbidden": "API访问被禁止。请检查您的API密钥权限。",
    "rateLimitExceeded": "超出速率限制。请稍后重试。",
    "serviceUnavailable": "翻译服务暂时不可用。请稍后重试。",
    "requestFailed": "API请求失败",
    "apiKeyError": "发生Gemini API密钥错误。请在设置中检查您的API密钥。",
    "translationFailed": "翻译失败",
    "requestTimeout": "翻译请求超时。请重试。",
    "failedPrefix": "翻译失败"
  },
  "utils": {
    "allUrlsFailed": "无法从所有URL获取版本信息",
    "invalidVersionFormat": "版本格式无效",
    "unknownError": "未知错误",
    "requestTimeout": "请求超时",
    "networkError": "网络连接失败",
    "securityRestriction": "浏览器安全策略限制",
    "serverError": "服务器响应错误",
    "terminalMac": "终端"
  },
  "modes": {
    "karaoke": "卡拉OK",
    "synced": "同步",
    "unsynced": "非同步"
  },
  "communityVideo": {
    "title": "社区视频推荐",
    "loading": "加载视频中...",
    "loadError": "加载视频失败",
    "noVideos": "暂无社区视频",
    "autoDetected": "自动",
    "startTime": "开始",
    "submittedBy": "提交者",
    "addVideo": "添加视频",
    "addVideoNoEmoji": "添加视频",
    "youtubeUrl": "YouTube 链接",
    "startTimeSeconds": "开始时间（秒）",
    "submit": "提交",
    "submitting": "提交中...",
    "submitted": "视频提交成功！",
    "updated": "视频更新成功！",
    "submitError": "视频提交失败",
    "invalidUrl": "无效的 YouTube 链接",
    "videoNotFound": "YouTube 视频不存在",
    "videoPrivate": "视频为私享或已删除",
    "validationError": "无法验证视频，请重试",
    "selectVideo": "选择视频",
    "preview": "预览",
    "apply": "应用",
    "applyShort": "应用",
    "applied": "视频已应用",
    "videoTitle": "视频标题",
    "loadingTitle": "正在获取标题...",
    "startTimeLabel": "首句歌词开始时间（秒）",
    "startTimeHint": "请输入 YouTube 视频中第一句歌词开始的时间",
    "delete": "删除",
    "deleteConfirm": "确定要删除这个视频吗？",
    "deleted": "视频已删除",
    "deleteError": "删除视频失败"
  },
  "close": "关闭",
  "cancel": "取消",
  "shareImage": {
    "title": "分享歌词图片",
    "subtitle": "创建并分享精美的歌词图片",
    "selectLyrics": "选择歌词",
    "selectLyricsHint": "点击选择要包含在图片中的歌词行（最多6行）",
    "template": "预设",
    "templates": {
      "cover": "封面模糊",
      "gradient": "渐变",
      "minimal": "极简",
      "glass": "毛璃璃",
      "story": "故事"
    },
    "advancedSettings": "详细设置",
    "sections": {
      "background": "背景",
      "cover": "专辑封面",
      "lyrics": "歌词",
      "layout": "布局",
      "other": "其他"
    },
    "copyrightWarning": "⚠️ 版权提示\n\n此歌词图片可能包含受版权保护的内容。\n\n• 请仅供个人使用\n• 请勿用于商业目的\n• 在社交媒体上分享时请尊重原创者\n\n是否继续？",
    "copyrightTitle": "版权提示",
    "copyrightDesc": "此歌词图片可能包含受版权保护的内容。",
    "copyrightPoint1": "请仅供个人使用",
    "copyrightPoint2": "请勿用于商业目的",
    "copyrightPoint3": "在社交媒体上分享时请尊重原创者",
    "copyrightConfirm": "同意并继续",
    "settings": {
      "backgroundType": "背景样式",
      "coverBlur": "模糊",
      "gradient": "渐变",
      "solid": "纯色",
      "backgroundBlur": "背景模糊",
      "backgroundOpacity": "背景暗度",
      "showCover": "专辑封面",
      "showTrackInfo": "歌曲信息",
      "coverPosition": "封面位置",
      "posLeft": "左侧",
      "posCenter": "居中",
      "coverSize": "封面大小",
      "coverRadius": "封面圆角",
      "coverBlur": "封面模糊",
      "showPronunciation": "发音",
      "showTranslation": "翻译",
      "lyricsAlign": "歌词对齐",
      "alignLeft": "左对齐",
      "alignCenter": "居中",
      "fontSize": "字体大小",
      "blockGap": "行间距",
      "aspectRatio": "图片比例",
      "imageWidth": "图片宽度",
      "padding": "边距",
      "showWatermark": "显示水印"
    },
    "preview": "预览",
    "actions": {
      "copy": "复制到剪贴板",
      "download": "下载",
      "share": "分享"
    },
    "copied": "✓ 已复制到剪贴板",
    "downloaded": "✓ 已下载",
    "shared": "✓ 已分享",
    "maxLinesReached": "最多可选择3行",
    "noSelection": "请选择歌词"
  },
  "setupWizard": {
    "welcome": {
      "title": "欢迎使用 ivLyrics！",
      "subtitle": "在 Spotify 上享受美丽的歌词体验",
      "features": {
        "lyrics": "实时同步歌词",
        "translation": "AI 翻译支持",
        "customization": "丰富的自定义选项"
      },
      "start": "开始"
    },
    "language": {
      "title": "语言选择",
      "subtitle": "请选择您使用的语言"
    },
    "apiKey": {
      "title": "Gemini API 密钥设置",
      "subtitle": "使用 AI 翻译功能需要 API 密钥",
      "guide": {
        "title": "如何获取 API 密钥",
        "step1": "访问 Google AI Studio",
        "step2": "点击 Get API Key",
        "step3": "创建新 API 密钥",
        "step4": "复制生成的密钥"
      },
      "getKey": "获取 API 密钥",
      "skip": "稍后设置",
      "placeholder": "输入 API 密钥...",
      "multipleKeysHint": "可以在设置中注册多个 API 密钥"
    },
    "theme": {
      "title": "基本设置",
      "subtitle": "选择歌词显示方式",
      "alignment": "对齐",
      "background": "背景效果",
      "backgrounds": {
        "colorful": "多彩",
        "gradient": "专辑封面",
        "blurGradient": "模糊渐变",
        "solid": "纯色",
        "video": "视频"
      }
    },
    "translationTip": {
      "title": "翻译设置指南",
      "subtitle": "翻译功能按语言单独设置",
      "pronunciation": "发音",
      "translation": "翻译",
      "description": "即使在 K-POP 歌曲中启用了翻译，也不会应用于 J-POP。请点击下方的转换按钮，分别设置每种语言的“发音”和“翻译”选项。"
    },
    "overlayTip": {
      "title": "桌面覆盖",
      "subtitle": "在桌面任何位置查看歌词",
      "description": "安装覆盖应用后，您可以在使用其他应用时查看歌词。您可以随时在设置中启用它。",
      "requiresApp": "使用此功能需要安装单独的覆盖应用程序。",
      "enabled": "启用覆盖",
      "downloadApp": "下载覆盖应用"
    },
    "nowPlayingTip": {
      "title": "正在播放面板歌词",
      "subtitle": "在右侧面板查看当前歌词",
      "description": "在 Spotify 的“正在播放”面板中显示当前歌词。无需全屏模式即可查看歌词。",
      "enabled": "显示面板歌词",
      "linesCount": "显示行数"
    },
    "complete": {
      "title": "准备就绪！",
      "subtitle": "现在可以开始使用 ivLyrics 了",
      "startNow": "立即开始",
      "openSettings": "打开设置"
    },
    "navigation": {
      "next": "下一步",
      "back": "上一步",
      "skip": "跳过"
    },
    "videoTest": {
      "title": "视频背景测试",
      "subtitle": "请检查下方的YouTube视频是否正常播放",
      "question": "是否显示需要登录才能播放视频？",
      "yes": "是",
      "no": "否",
      "helperRequired": "如果是，您需要安装助手程序。",
      "helperDesc": "助手程序可以让您无需YouTube登录即可播放视频。",
      "installHelper": "安装助手程序",
      "skip": "跳过"
    },
    "videoHelperTest": {
      "title": "助手程序测试",
      "subtitle": "验证助手程序是否正常工作",
      "enableHelper": "启用助手程序",
      "testVideo": "测试视频播放",
      "testing": "测试中...",
      "success": "助手程序运行正常！",
      "failed": "无法连接到助手程序。请确保程序正在运行。",
      "downloading": "正在下载视频: {percent}%",
      "skip": "跳过"
    }
  },
  tmi: {
    getApiKeyDesc: "获取 Gemini API 密钥",
    getApiKeyInfo: "用于获取 TMI。可从 Google AI Studio 免费获取。",
    viewInfo: "点击查看歌曲详细信息",
    requireKey: "需要在设置中配置 Gemini API 密钥",
    settingTitle: "Gemini API 密钥",
    settingDesc: "用于加载歌曲 TMI/趣闻内容。",
    title: "TMI",
    didYouKnow: "你知道吗？",
    close: "关闭",
    cancel: "取消",
    loading: "正在查找趣闻...",
    noData: "暂无此歌曲的 TMI。",
    clickForTMI: "点击查看 TMI",
    clickToClose: "点击 ✕ 或专辑区域返回",
    regenerate: "重新生成 TMI",
    errorFetch: "获取 TMI 时发生错误。",
    errorQuota: "API 配额已超出。",
    errorQuotaHint: "请稍后重试或在设置中输入其他 API 密钥。",
    disclaimer: "此内容由AI处理互联网信息后生成，可能包含不准确的信息。",
    verified: "已验证",
    unverified: "未验证",
    verifiedCount: "已验证",
    sources: "来源",
    verifiedSources: "已验证的来源",
    relatedSources: "相关来源",
    otherSources: "其他来源",
    confidenceVeryHigh: "极高可信度",
    confidenceHigh: "高可信度",
    confidenceMedium: "中可信度",
    confidenceLow: "低可信度",
    confidenceNone: "无来源"
  },
  notice: {
    confirm: "确认",
    next: "下一个",
    dismissAll: "全部关闭"
  },
  syncCreator: {
    title: "创建卡拉OK同步",
    buttonTooltip: "创建卡拉OK同步",
    clickHereHint: "点击此处同步歌词！",
    loading: "加载中...",
    loadLyrics: "加载歌词",
    loadingLyrics: "正在加载歌词...",
    selectProvider: "请选择提供商并加载歌词",
    noLyrics: "未找到歌词",
    loadError: "加载歌词时出错",
    currentLine: "当前行",
    nextLine: "下一行",
    progress: "进度",
    linesCompleted: "已完成行数",
    chars: "字符",
    reset: "重置",
    firstLine: "首行",
    prevLine: "上一行",
    nextLineBtn: "下一行",
    recordMode: "录制模式",
    stopRecord: "停止录制",
    previewMode: "预览模式",
    stopPreview: "停止预览",
    idleMode: "空闲",
    recordLine: "快速录制行",
    submit: "提交",
    submitting: "正在提交...",
    noSyncData: "没有同步数据",
    incompleteConfirm: "部分行尚未同步。确定要提交吗？",
    submitSuccess: "同步数据已提交！审核后将生效。",
    submitError: "提交同步数据失败",
    dragHint: "拖动以记录每个字符的时间。向左拖动取消。",
    globalOffset: "全局偏移",
    synced: "已同步",
    notSynced: "未同步",
    deleteLine: "删除此行",
    back: "关闭"
  }
};
